...
use crate::{consts::{PROJECT_FILE, PROJECT_SRC, PROJECT_TARGET, PROJECT_TMP, TEMP_FILE}, utils::{awrite_file, get_or_make_dir}};
...
pub fn get_project_source_files(path: &path::Path, ext: &str) -> anyhow::Result<Vec<path::PathBuf>> {
        ...
        for fentry in fs::read_dir(current_dir)? {
            ...
            if ftype.is_dir() {
                ...
                let fname = fname.to_str()
                    .ok_or(anyhow::anyhow!("Source file at {} contains invalid unicode characters in its name", fpath.to_string_lossy()))?;
