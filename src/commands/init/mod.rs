...
use template::{template_config, template_java_code};
    ...
    fn handle(&mut self, ctx: super::CommandCtx) -> anyhow::Result<()> {
        ...
        match dir_empty(&path) {
            ...
            Ok(false) => {
                ...
                if !force_overwrite {
                    let resp =
                        prompt("The directory is not empty, do you still want to proceed? (Y) ");
            ...
            Err(_) => {
                return Err(anyhow::anyhow!(
                    "The provided directory either doesn't exist or is not a directory file."
                ));
            }
        }
        ...
        if !force_overwrite && is_project_dir(&path).expect("The directory is safe to access") {
            let resp = prompt(
                "The directory already contains a jana project (Jana.toml). Do you still want to proceed? (Y) ",
            );
            if !resp.starts_with("Y") {
                println!("Aborting...");
                return Ok(());
        ...
        let package = match package {
            ...
            None => loop {
                let package =
                    prompt("Type the full name of the package (example: org.example.com): ")
                        .trim()
                        .to_owned();
        ...
        {
            let file_name = if matches!(kind, ProjectKind::Bin) {
                "Main.java"
            } else {
                "Lib.java"
            };
        ...
        match Git::get() {
            ...
            Err(_) => println!(
                "No git command found, the created project was initialized without a github repository."
            ),
        }
