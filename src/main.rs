...
fn main() -> anyhow::Result<()> {
    ...
    let _exe = args.next().expect("Path to the executable is always present");
    ...
    if handlers.has_command(&command) {
        ...
        println!("No command \"{command}\" found. Run `help` to get the entire list of available commands.");
